  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans text-slate-900">
      ...
      <main className="max-w-7xl mx-auto p-6 md:p-10 space-y-8">
        ...
        {/* WELCOME BANNER */}
        <section className="relative rounded-[2.5rem] overflow-hidden bg-slate-900 h-64 flex items-end">
          ...
          <div className="relative z-10 p-8 md:p-12 w-full flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <div className="flex items-center gap-2 text-emerald-400 font-bold text-xs uppercase tracking-[0.2em] mb-2">
                <Calendar size={14} /> {new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
              </div>
              <h2 className="text-4xl font-black text-white tracking-tight">Welcome, {user?.name?.split(" ")[0]}</h2>
              <p className="text-slate-300 font-medium opacity-80 mt-1 text-lg">Your climate impact intelligence is ready.</p>
            </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white p-8 md:p-10 rounded-[2.5rem] border border-slate-100 shadow-sm">
            <div className="flex justify-between items-center mb-10">
              ...
              <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 px-3 py-1 rounded-full">Historical Data</div>
            </div>
          ...
          <div className="bg-emerald-600 rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl shadow-emerald-200">
            ...
            <button onClick={() => navigate("/predict")} className="w-full bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/20 py-4 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all">
...
function NavBtn({ icon, label, active, onClick }) {
  return (
    <button 
      onClick={onClick}
      className={`flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold text-sm transition-all ${active ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
    >
      {icon} {label}
    </button>
  );
...
function PremiumCard({ title, value, trend, sub }) {
  return (
    <div className="bg-white p-8 rounded-[2.2rem] border border-slate-100 shadow-sm hover:shadow-md transition-all group">
      <div className="flex justify-between items-start mb-4">
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{title}</p>
        {trend !== undefined && (
          <div className={`flex items-center font-bold text-xs ${trend >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
            {trend >= 0 ? <ArrowUpRight size={14}/> : <ArrowDownRight size={14}/>}
            {Math.abs(trend)}%
          </div>
        )}
      </div>
      <h2 className="text-3xl font-black text-slate-900 mb-1 tracking-tight group-hover:text-emerald-600 transition-colors">
        {value}
      </h2>
...
function PremiumCardDark({ title, value, sub }) {
  return (
    <div className="bg-slate-900 p-8 rounded-[2.2rem] shadow-xl relative overflow-hidden">
      ...
      <p className="text-[11px] font-bold text-emerald-500/60 uppercase">{sub}</p>
    </div>
  );
}
